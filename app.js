...
const updatePage = async () => {
  ...
  function filterSugar(fruit) {
    return fruit.nutritions.sugar > 15
  }

  const filteredFruits = fruitsArray.filter(filterSugar)
  console.log(filteredFruits)

  const fruitElement = document.createElement('div');
  fruitElement.innerHTML = "Fruits with sugar > 15 are: "
  for (let i = 0; i < filteredFruits.length; i++) {
    fruitElement.innerHTML += filteredFruits[i].name
